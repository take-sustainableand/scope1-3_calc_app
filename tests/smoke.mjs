...
// CSV 取込で投入したベースラインを smoke で守る（消失・上書きを検知）。
// 件数の最小値と、代表 activity（電力 2026-02-09 / 都市ガス 2026-03-18）の存在をチェック。
check("JSON の activities が CSV 取込分を保持（電力12件・都市ガス12件以上）",
  stateJson.activities.filter((a) => a.factorId === "f-electricity").length >= 12 &&
  stateJson.activities.filter((a) => a.factorId === "f-citygas").length >= 12
);
check("JSON に CSV 取込の代表レコード（電力 2026-02-09 617 kWh）が残っている",
  stateJson.activities.some((a) => a.factorId === "f-electricity" && a.date === "2026-02-09" && a.amount === 617)
);
check("JSON に CSV 取込の代表レコード（都市ガス 2026-03-18 56 m3）が残っている",
  stateJson.activities.some((a) => a.factorId === "f-citygas" && a.date === "2026-03-18" && a.amount === 56)
);
