...
- **活動データのみ削除**: 入力した activities だけを空にします。 factors（原単位マスタ）は残ります。
- **シードに戻す**: factors / activities の localStorage キーを削除し、 `data/scarbon-state.json` のシードデータから再ロードします。間違って原単位を全消ししたり、 過去の空配列が永続化されて「原単位がまだありません」と表示されるケースを救済できます。

トークン・GitHub 接続情報・テーマ設定はどちらの操作でも残ります（トークンは別ボタンで削除できます）。

## GitHub Pages へのデプロイ

このリポジトリは <https://github.com/take-sustainableand/scope1-3_calc_app> をホストとして利用する想定です。本番公開 URL は <https://sustainableand.com/scope1-3_calc_app/> （GitHub Pages のカスタムドメイン）です。

1. GitHub のリポジトリ画面で **Settings → Pages** を開く
2. **Source** を `Deploy from a branch` に設定
3. **Branch** を `main` の `/ (root)` に設定して保存
4. （カスタムドメインを使う場合）**Custom domain** に `sustainableand.com` を入力。リポジトリ直下の `CNAME` ファイルにも同じ値を記載しておくと、再デプロイ時にドメイン設定が失われません。
5. 数十秒〜数分後に <https://sustainableand.com/scope1-3_calc_app/> （カスタムドメイン未設定時は `https://take-sustainableand.github.io/scope1-3_calc_app/`）が公開される
